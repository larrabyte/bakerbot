...
class Starboard(commands.Cog):
    ...
    async def ensure_config(self, identifier: int) -> database.GuildConfiguration:
        """"Returns the starboard configuration for a guild (inserting a new one if necessary)."""
        if (config := await database.GuildConfiguration.get(self.bot.db, identifier)) is not None:
        ...
        config = database.GuildConfiguration.new(identifier)
